    const publicationRoot = "../test/archive_data/v2/web/data";
    const manifestUrl = `${publicationRoot}/manifest.json`;
    ...
    function publishedUrl(suffix) {
      ...
      return file ? `${publicationRoot}/${file.path}` : null;
    }
    async function fetchPublicationPath(path) {
      const response = await fetch(`${publicationRoot}/${path}?t=${Date.now()}`, { cache: "no-store" });
    ...
    async function renderPredictions(target) {
      ...
      try {
        ...
        const rows = predictionCache.get(target)?.rows ?? [];
        empty.hidden = rows.length > 0;
        empty.textContent = "予測データはまだありません";
    ...
    async function loadManifest() {
      ...
      try {
        ...
        showNotice(`test/archive_data/v2/web/data/manifest.jsonを取得できません: ${error.message}`);
        renderFiles();
        drawChart();
        drawTopNChart();
        drawModelChart();
    ...
    document.getElementById("apply-date-range").addEventListener("click", () => {
      ...
      const start = parseCompactDate(startInput.value.trim());
      const end = parseCompactDate(endInput.value.trim());
      startInput.setCustomValidity(start ? "" : "YYYYMMDDの8桁で正しい開始日を入力してください");
      endInput.setCustomValidity(end ? "" : "YYYYMMDDの8桁で正しい終了日を入力してください");
      if (!start) { startInput.reportValidity(); return; }
      if (!end) { endInput.reportValidity(); return; }
      if (start > end) {
        endInput.setCustomValidity("終了日は開始日以降を指定してください");
