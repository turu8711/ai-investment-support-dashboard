    const manifestUrl = "../test/archive_data/v1/web/data/manifest.json";
    ...
    function manifestFile(suffix) { return manifest?.files?.find((item) => item.path.endsWith(suffix)); }
    function publishedUrl(suffix) {
      const file = manifestFile(suffix);
      return file ? `../test/archive_data/v1/web/data/${file.path}` : null;
    }
    async function fetchPublished(suffix) {
      const url = publishedUrl(suffix);
      if (!url) return null;
    ...
    async function loadManifest() {
      ...
      try {
        ...
        showNotice(`test/archive_data/v1/web/data/manifest.jsonを取得できません: ${error.message}`);
        renderFiles();
        drawChart();
        drawTopNChart();
        drawModelChart();
